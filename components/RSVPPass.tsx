...
const RSVPPass: React.FC<RSVPPassProps> = ({ data, onWriteLetter }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full px-4 animate-pop">
      ...
      {/* 
         THE "POLKA CHIC" CARD
         Container: Black background with pink dots (The "Vibe")
         Card: White paper, 4:3 Ratio, Minimal Text
      */}
      <div className="w-full flex flex-col items-center mb-6">
        
        {/* This wrapper provides the "Black & Pink" context border without looking like a suitcase */}
        <div 
            className="p-3 shadow-2xl relative"
            style={{
                backgroundColor: '#000',
                backgroundImage: 'radial-gradient(#f9a8d4 1.5px, transparent 1.5px)', // Pink dots on Black
                backgroundSize: '20px 20px',
                // transform: 'rotate(-1deg)' // REMOVED ROTATION -> Straightened
            }}
        >
            {/* The Actual Card (White Paper) - Locked 4:3 Ratio */}
            <div 
                ref={passRef}
                className="bg-[#fffdfa] relative flex flex-col items-center text-center px-6 py-8 shadow-inner"
                style={{ 
                    width: '240px',          // Fixed width (Small)
                    height: '320px',         // Fixed height (4:3 Ratio)
      ...
      {/* 
        BUTTONS - Updated to ROUNDED-FULL with Icons on Right
      */}
      <div className="w-full max-w-[240px] space-y-3">
        {/* Primary Action */}
        <button 
            onClick={onWriteLetter}
            className="w-full py-3.5 bg-gray-900 text-white rounded-full font-cute font-bold text-[10px] uppercase tracking-[0.15em] shadow-xl hover:bg-black active:scale-95 transition-all flex items-center justify-center gap-2 group"
        >
        ...
        <button 
            ...
            className="w-full py-3 bg-white/60 backdrop-blur-sm text-gray-600 rounded-full font-cute font-bold text-[10px] uppercase tracking-[0.15em] hover:bg-white hover:text-gray-900 transition-all flex items-center justify-center gap-2 group"
        >
